package com.andy.seata.at;

import lombok.extern.slf4j.Slf4j;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.jdbc.core.JdbcTemplate;

/**
 * @Author Andy
 * @Date: 2022/03/15/ 16:07
 * @Description
 */
@Slf4j
@SpringBootTest
public class ATTest {

    @Autowired
    private BizService bizService;

    @Autowired
    private JdbcTemplate jdbcTemplateA;

    @Autowired
    private JdbcTemplate jdbcTemplateB;

    @Test
    public void test() throws Exception{

        int balanceA = jdbcTemplateA.queryForObject("select money from user_money_a where id = 1", Integer.class);
        int balanceB = jdbcTemplateB.queryForObject("select money from user_money_b where id = 1", Integer.class);
        log.info("当前余额: A: " + balanceA + "   B: " + balanceB + "   total：" + (balanceA + balanceB));
        log.info("--------------------------------------------------------------------------------------------");
        log.info("即将开始模拟交易20次, 每次会从 A 转账 300 到 B, 转入/转出时会随机发生异常, 金额不足时也会抛出异常");
        log.info("--------------------------------------------------------------------------------------------\n\n\n");
        Thread.sleep(3000);

        for (int i = 1; i <= 20; ++i) {
            log.info("第[" + i + "] 次交易开始");
            try {
                bizService.doTransfer(300);
                log.info("第[" + i + "] 次交易结束! ");
            } catch (Exception e) {
                log.info("第[" + i + "] 次交易异常! 异常原因 " + e.getMessage() + ". 金额已经回滚.");
            }
            balanceA = jdbcTemplateA.queryForObject("select money from user_money_a where id = 1", Integer.class);
            balanceB = jdbcTemplateB.queryForObject("select money from user_money_b where id = 1", Integer.class);
            log.info("当前余额校验: A: " + balanceA + "   B: " + balanceB + "   total：" + (balanceA + balanceB));
            log.info("--------------------------------------------------------------------------------------------");
            Thread.sleep(1000);
        }

        log.info("\n\n demo 运行结束");

    }
}
